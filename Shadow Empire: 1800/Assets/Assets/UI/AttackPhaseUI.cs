...
public class AttackPhaseUI : MonoBehaviour
{
    ...
    void EnsureNodes()
    {
        ...
        foreach (string key in staleKeys) buttons.Remove(key);
    ...
    void OnNodeClicked(string territoryName)
    {
        ...
        {
            // NEW: reject non-adjacent targets
            List<string> validNeighbours = ProvinceGraph.GetAttackNeighbours(selectedSource.name);
            if (!validNeighbours.Contains(territory.name))
            {
                gm.LogMessage($"{territory.name} is not adjacent to {selectedSource.name}.");
                return;
            }

            TerritoryData source = selectedSource;
            TerritoryData target = territory;
            if (GameUIManager.Instance != null && player.isPlayerControlled)
            {
                GameUIManager.Instance.BuildAmbushMinigame(source, target, bonus =>
                {
                    gm.pendingAmbushAttackBonus = bonus;
                    gm.IsWaitingForAttackSelection = false;
                    Hide();
                    gm.pendingAttackSource = source;
                    gm.pendingAttackTarget = target;
                    TerritoryGraphRenderer.Instance?.ShowPendingAttack(source.name, target.name);
                    gm.LogMessage($"Attack phase: {source.name} attacks {target.name}.");
                });
            }
            else
            {
    ...
    public void ResetSelection()
    {
        selectedSource = null;
        if (GameManager.Instance != null)
            GameManager.Instance.pendingAttackSource = null;
        TerritoryGraphRenderer.Instance?.HighlightAttackOptions(null);
        Refresh();
    }

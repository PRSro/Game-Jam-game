...
public class CutsceneManager : MonoBehaviour
{
    ...
    IEnumerator PlayCutsceneRoutine(int factionIndex)
    {
        ...
        // Application.streamingAssetsPath resolves correctly on all platforms.
        string fsPath = System.IO.Path.Combine(
            Application.streamingAssetsPath, "Cutscenes",
            cutscenePaths[factionIndex] + ".webm");

        if (!System.IO.File.Exists(fsPath))
        {
            Debug.LogError(
                "[CutsceneManager] VIDEO FILE MISSING — expected at: " + fsPath +
                "\nMake sure .webm files are in StreamingAssets/Cutscenes/.");
            onComplete?.Invoke();
            yield break;
        }

        // Build the file:// URL - encode spaces only, keep colons literal
        string unixPath = fsPath.Replace("\\", "/");
        // Only encode spaces - Unity/GStreamer on Linux rejects %3A for colons in file paths
        string encodedPath;
        if (unixPath.StartsWith("/"))
            encodedPath = "file://" + unixPath.Replace(" ", "%20");
        else
            encodedPath = "file:///" + unixPath.Replace(" ", "%20");

        cutsceneUrls[factionIndex] = encodedPath;
        Debug.Log("[CutsceneManager] Video URL resolved to: " + encodedPath);
        Debug.Log("[CutsceneManager] File exists check: " + System.IO.File.Exists(fsPath));

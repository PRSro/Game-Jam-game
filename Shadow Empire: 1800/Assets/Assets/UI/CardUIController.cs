...
public class CardUIController : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    public void SetCard(CardData card, Color factionColor)
    {
        ...
        if (artImage != null)
        {
            ...
            // Priority 1: stable art key maps to root Cards folder (card1.png..card24.png)
            if (!string.IsNullOrEmpty(card.artKey))
                artTex = Resources.Load<Texture2D>("Cards/" + card.artKey);

            // Priority 2: archive folder by numeric ID (legacy, only 1.png exists)
            if (artTex == null)
                artTex = Resources.Load<Texture2D>("Cards/Archive/" + card.cardId);

            // Priority 3: archive folder by art key
            if (artTex == null && !string.IsNullOrEmpty(card.artKey))
                artTex = Resources.Load<Texture2D>("Cards/Archive/" + card.artKey);

            // Fallback: try lower-case art key
            if (artTex == null && !string.IsNullOrEmpty(card.artKey))
            {
                artTex = Resources.Load<Texture2D>("Cards/" + card.artKey.ToLower());
                if (artTex == null)
                    Debug.LogWarning($"[CardUI] Missing art: Resources/Cards/{card.artKey} — using placeholder. Place card1.png–card24.png in Assets/Resources/Cards/");
            }

            // Priority 4: procedural placeholder (existing fallback)
            if (artTex == null)
            {
        ...
        if (descText != null)
        {
            ...
            else
            {
                switch (card.cardType)
                {
                    case CardType.ATTACK: descText.text = "Territory Attack \u2014 select source then target on map"; break;

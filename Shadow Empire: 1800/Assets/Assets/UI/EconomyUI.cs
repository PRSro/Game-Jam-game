...
public class EconomyUI : MonoBehaviour
{
    ...
    public void Build(Transform parent)
    {
        ...
        goldDisplayText = MakeUIText(panel.transform, "GoldDisplay", 10, FontStyle.Bold,
            Parchment, new Vector2(0.60f, 0.83f), new Vector2(0.88f, 0.97f)).GetComponent<Text>();
        MakeUIText(panel.transform, "CurrencyNote", 9, FontStyle.Italic,
            new Color(0.7f, 0.9f, 0.5f), new Vector2(0.02f, 0.74f), new Vector2(0.60f, 0.82f))
            .GetComponent<Text>().text = "Costs GOLD \u2014 buy faction cards and upgrades";
    ...
    public void Refresh()
    {
        ...
        if (goldDisplayText != null && player != null)
            goldDisplayText.text = $"GOLD: {player.gold}  (+{player.goldIncome}/turn)";
        RebuildRows();
    }
    ...
    public void Show()
    {
        ...
        RebuildRows();
        if (goldDisplayText != null && GameManager.Instance != null)
        {
            FactionData player = GameManager.Instance.GetPlayerFaction();
            if (player != null)
                goldDisplayText.text = $"GOLD: {player.gold} (+{player.goldIncome}/turn)";

...
public static class CardDatabase
{
    ...
    public static void Reset() { allCards = null; _nextCardId = 0; }
    ...
    static CardData MakeCard(string name, string desc, int power, CardType type, int factionId, string flavor, CardRarity? rarityOverride = null)
    {
        ...
        card.goldCost = card.rarity switch
        {
            CardRarity.COMMON => 1,
        ...
        if (card.cardType == CardType.ATTACK) card.goldCost++;
        if (card.cardType == CardType.BLACK_MARKET) card.goldCost++;
        return card;
    }

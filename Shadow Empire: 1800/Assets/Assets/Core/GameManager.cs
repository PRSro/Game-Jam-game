...
public class GameManager : MonoBehaviour
{
    ...
    public int lastPlayerNeutralExpansionTurn = -99; // track when player last expanded into neutral
    ...
    public int pendingAttackerTroopCount = 0;
    public int pendingAmbushAttackBonus = 0;
    public int pendingCardPowerBonus = 0;
    ...
    void InitializeGame()
    {
        ...
        if (AudioManager.Instance != null)
            AudioManager.Instance.SetPlayerFaction(playerFactionId);
    ...
    void AssignStartingTerritories()
    {
        ...
        {
            { 0, new List<string> { "Paris", "Amsterdam", "Geneva" } },
            { 1, new List<string> { "London", "Rome", "Constantinople" } },
            { 2, new List<string> { "Berlin", "Vienna", "Stockholm" } },
            { 3, new List<string> { "Naples", "Florence" } }
        };
    ...
    IEnumerator StartPlayerTurn()
    {
        if (currentTurn > MAX_TURNS)
        {
            ...
            if (winner == null)
            {
                LogMessage("All factions have collapsed — the Age of Shadows ends in chaos.");
        ...
        if (HistoricalEventManager.TryTriggerEvent(this))
        {
            ...
            {
                if (HistoricalEventManager.ActiveEvent != null && HistoricalEventManager.ActiveEvent.requiresPlayerChoice)
                    HistoricalEventManager.ResolveChoice(this, true);
        ...
        HistoricalEventManager.TryTriggerCrisisEvent(this);
        ...
        if (!player.isEliminated)
        {
            ...
            float timeout = PHASE_TIME_LIMIT;
            float elapsed = 0f;
            while (IsWaitingForReinforcements && elapsed < timeout)
            {
                elapsed += Time.deltaTime;
                yield return null;
            }
        ...
        {
            t.skipReinforceNextTurn = false;
            t.suppressAttackNextTurn = false;
        }
        ...
        bool awardCardsThisRound = currentTurn % 3 == 1;
        if (awardCardsThisRound)
        {
            ...
            foreach (FactionData f in factions)
            {
                if (!f.isEliminated && !f.isPlayerControlled)
                {
                    ...
                    CardData drawn = DeckManager.DrawCard(f);
            ...
            {
                CardData playerDrawn = DeckManager.DrawCard(player);
                if (playerDrawn != null)
                {
        ...
        {
            int nextDrawTurn = ((currentTurn - 1) / 3 + 1) * 3 + 1;
        ...
        LogMessage($"Your turn — play up to {MAX_CARDS_PER_TURN} cards, then press END TURN. ({PHASE_TIME_LIMIT}s timer)");
    ...
    void AutoAssignReinforcements(FactionData faction)
    {
        ...
        {
            if (t.skipReinforceNextTurn) continue;
    ...
    public void AssignReinforcementToTerritory(TerritoryData territory, int count)
    {
        ...
        {
            LogMessage($"Cannot reinforce {territory.name}: not controlled by player.");
            return;
        }
        if (territory.skipReinforceNextTurn)
        {
            LogMessage($"{territory.name} cannot be reinforced this turn.");
            return;
        }
    ...
    public int ConsumePendingCardPowerBonus()
    {
        int bonus = pendingCardPowerBonus;
        pendingCardPowerBonus = 0;
        return bonus;
    }
    ...
    {
        int dice = this.pendingAttackerDiceCount;
        int troops = this.pendingAttackerTroopCount;
        PlayedCard played = new PlayedCard(card, source, null, sourceTerritory, targetTerritory, dice, troops);
        resolutionQueue.Enqueue(played);
        this.pendingAttackerDiceCount = 0;
        this.pendingAttackerTroopCount = 0;
    }
    ...
    IEnumerator AttackPhaseThenResolution()
    {
        ...
        while (attacksThisPhase < 2 && player != null && !player.isEliminated && HasAttackPhaseOptions(player))
        {
            SetGameState(GameState.ATTACK_PHASE);
            LogMessage($"ATTACK: Choose territory attack route, or wait to skip. (Attack {attacksThisPhase + 1}/2)");
            ...
            {
                LogMessage("No target selected — click an adjacent enemy territory.");
                pendingAttackSource = null;
                IsWaitingForAttackSelection = true;
                elapsed = 0f;
                AttackPhaseUI.Instance?.ResetSelection();
                continue;
            }
    ...
    IEnumerator ResolutionAndAIPhase()
    {
        ...
        while (resolutionQueue.Count > 0)
        {
            ...
            if (played.source == null) { continue; }
            ...
            if (!played.source.isPlayerControlled && played.card != null)
            {
                ...
                if (AudioManager.Instance != null)
                    AudioManager.Instance.OnEnemyFactionActing(played.source.factionId);
            }
        ...
        if (AudioManager.Instance != null)
            AudioManager.Instance.OnResolutionComplete();
        ...
        while (IsWaitingForFortify && fortifyElapsed < fortifyTimeout)
        {
        ...
        yield return new WaitForSeconds(0.5f);
        playerCardsThisResolution.Clear();   // Clear AFTER combo detection, not before
        allCardsThisResolution.Clear();
    ...
    {
        if (played.sourceTerritory == null || played.targetTerritory == null)
        {
            Debug.LogWarning("[GameManager] ResolveTerritoryAttack skipped: null territory.");
            yield break;
        }
        ...
        attackBonusDice += crusadeBonus;

        if (target.holyWarMarkedByFaction == played.source.factionId)
        {
            defenseBonusDice = 0;
            preBattleSabotage += 5;
            target.holyWarMarkedByFaction = -1;
            LogMessage($"{played.source.factionName}'s Holy War ignores defense and deals +5.");
        }

        if (target.defensePenalty < 0)
        {
            defenderShield = Mathf.Max(0, defenderShield + target.defensePenalty);
            LogMessage($"{target.name}'s planted-agent defense penalty applies ({target.defensePenalty}).");
        }

        int comboBonus = card != null ? GetComboPower(card) : 0;
        attackBonusDice += pendingAmbushAttackBonus;
        pendingAmbushAttackBonus = 0;
        int chosenDice = played.chosenDice > 0 ? played.chosenDice : 0;
        int committedTroops = played.chosenTroops;
        CombatResult result = CombatManager.ResolveTerritoryBattle(
            ...
            maxRounds: card != null ? card.power + comboBonus : 3,
            attackerChosenDice: chosenDice, attackerCommittedTroops: committedTroops);
        ...
        if (result.attackerWon && result.territoryCaptured)
        {
            ...
            if (!played.source.isPlayerControlled)
                played.source.territoryAttackWins++;
        }
    ...
    public void CheckEliminations()
    {
        foreach (FactionData faction in factions)
        {
            ...
            bool legitimacyCollapsed = faction.legitimacy <= 0f;
            if (noTerritories || powerDepleted || legitimacyCollapsed)
            {
                faction.isEliminated = true;
                faction.power = 0;
                string reason;
                if (noTerritories)
                    reason = $"{faction.factionName} has lost all territories and is ELIMINATED!";
                else if (powerDepleted)
                    reason = $"{faction.factionName}'s power has collapsed \u2014 ELIMINATED!";
                else if (legitimacyCollapsed)
                    reason = $"{faction.factionName}'s legitimacy has shattered \u2014 ELIMINATED!";
                else
                    reason = $"{faction.factionName} has been fully exposed \u2014 ELIMINATED!";
                LogMessage(reason);
    ...
    bool CheckWinCondition()
    {
        ...
        // Check territorial victory first (first-come-first-serve remains for this one)
        foreach (FactionData faction in factions)
        {
        ...
        // Collect qualifying factions for each non-territorial victory condition
        List<FactionData> dominanceCandidates = new List<FactionData>();
        List<FactionData> influenceCandidates = new List<FactionData>();
        List<FactionData> shadowCoupCandidates = new List<FactionData>();
        List<FactionData> conspiracyCandidates = new List<FactionData>();

        foreach (FactionData faction in factions)
        {
            if (faction.isEliminated) continue;

            // Dominance victory: influence >= 100 && power >= 90 && territories.Count >= 8
            if (faction.influence >= 100 && faction.power >= 90 && faction.territories.Count >= 8)
                dominanceCandidates.Add(faction);

            // Influence victory: influence >= 100 && currentTurn >= 60 && territories.Count >= 6
            if (faction.influence >= 100 && currentTurn >= 60 && faction.territories.Count >= 6)
                influenceCandidates.Add(faction);

            // Shadow coup: secrets >= 60 && territories.Count >= 5 && currentTurn >= 30
            if (faction.secrets >= 60 && faction.territories.Count >= 5 && currentTurn >= 30)
                shadowCoupCandidates.Add(faction);

            // Conspiracy victory: secrets >= 75 && at least 2 eliminations
            if (faction.secrets >= 75)
            {
                int elimCount = 0;
                foreach (FactionData f in factions)
                    if (f.isEliminated) elimCount++;
                if (elimCount >= 2)
                    conspiracyCandidates.Add(faction);
            }
        }

        // Check each victory condition with tie-breaking (highest influence + territories.Count + power wins)
        FactionData winner = null;
        string victoryMessage = "";

        if (dominanceCandidates.Count > 0)
        {
            winner = dominanceCandidates.OrderByDescending(f => f.influence + f.territories.Count + f.power).First();
            victoryMessage = $"DOMINANCE VICTORY: {winner.factionName} controls the shadows of Europe!";
        }
        else if (influenceCandidates.Count > 0)
        {
            winner = influenceCandidates.OrderByDescending(f => f.influence + f.territories.Count + f.power).First();
            victoryMessage = $"INFLUENCE VICTORY: {winner.factionName}'s influence is absolute!";
        }
        else if (shadowCoupCandidates.Count > 0)
        {
            winner = shadowCoupCandidates.OrderByDescending(f => f.influence + f.territories.Count + f.power).First();
            victoryMessage = $"SHADOW COUP: {winner.factionName} executes a devastating secret coup!";
        }
        else if (conspiracyCandidates.Count > 0)
        {
            winner = conspiracyCandidates.OrderByDescending(f => f.influence + f.territories.Count + f.power).First();
            victoryMessage = $"CONSPIRACY VICTORY: {winner.factionName}'s secrets are absolute!";
        }
        ...
        // Check for last faction standing
        int aliveCount = factions.Count(f => !f.isEliminated);
        bool playerAlive = factions.Any(f => !f.isEliminated && f.isPlayerControlled);
    ...
    void CheckCardCombos()
    {
        ...
        var realCards = playerCardsThisResolution.Where(c => c != null).ToList();
        if (realCards.Count < 2) return;

        var groups = realCards.GroupBy(c => c.cardType).ToList();
    ...
    public void RestartGame()
    {
        ...
        resolutionQueue.Clear();
        currentTurn = 1;
        lastPlayerNeutralExpansionTurn = -99;
        pendingAttackSource = null;
        pendingAttackTarget = null;
        IsWaitingForReinforcements = false;
        IsWaitingForEventDismiss = false;
    ...
    void ApplyTerritoryIncome(FactionData faction)
    {
        ...
        faction.secrets = Mathf.Min(75, faction.secrets + totalFarming);
        faction.influence = Mathf.Min(100, faction.influence + totalInfluence + 1);
        faction.power = Mathf.Min(100, faction.power + totalPower);

        int ownedCount = 0;
        foreach (TerritoryData t in territories)
            if (t.controlledBy == faction.factionId) ownedCount++;
        faction.goldIncome = Mathf.Max(1, ownedCount + faction.goldIncomeBonus);
        faction.gold += faction.goldIncome;
        OnGoldChanged?.Invoke(faction);
    }
    ...
    void ApplyFactionPassives()
    {
        foreach (FactionData faction in factions)
        {
            if (faction == null || faction.isEliminated) continue;

            switch (faction.factionId)
            {
                case 0:
                    faction.influence = Mathf.Min(100, faction.influence + 2);
                    LogMessage("Illuminati Enlightened: +2 influence.");
                    break;
                case 1:
                    foreach (TerritoryData t in territories)
                    {
                        if (t.controlledBy == faction.factionId && t.troops < 8)
                            t.troops += 1;
                    }
                    LogMessage("Knights Templar Fortified: owned territories gain troops.");
                    break;
                case 2:
                    int lodgeSecrets = territories.Count(t => t.controlledBy == faction.factionId);
                    faction.secrets = Mathf.Min(75, faction.secrets + lodgeSecrets);
                    LogMessage($"Freemason Lodge Network: +{lodgeSecrets} secrets.");
                    break;
            }
        }
    }
    ...
    bool HasAttackPhaseOptions(FactionData faction)
    {
        ...
        {
            if (source.controlledBy != faction.factionId) continue;
            if (source.troops < 2) continue;
            // Must have at least one adjacent enemy or neutral territory
            foreach (string adjName in ProvinceGraph.GetAttackNeighbours(source.name))
            {
                TerritoryData adj = territories.Find(t => t.name == adjName);
                if (adj != null && adj.controlledBy != faction.factionId)
                    return true;
            }
        }
        return false;
    ...
    {
        List<TerritoryData> result = new List<TerritoryData>();
        foreach (string adjName in ProvinceGraph.GetAttackNeighbours(target.name))
        {
            TerritoryData t = territories.Find(td => td.name == adjName);
    ...
    bool IsConnectedThroughFriendlyTerritories(TerritoryData a, TerritoryData b, int factionId)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (string adj in ProvinceGraph.GetAttackNeighbours(currentTerritory.name))
            {
...
public struct PlayedCard
{
    ...
    public int chosenDice;
    public int chosenTroops;
    ...
    public PlayedCard(CardData card, FactionData source, FactionData target, TerritoryData sourceTerritory, TerritoryData targetTerritory, int chosenDice = 0, int chosenTroops = 0)
    {
        this.card = card;
        this.source = source;
        this.target = target;
        ...
        this.chosenTroops = chosenTroops;
    }
}

...
public class HistoricalEvent
{
    ...
    public System.Action<GameManager> applyChoiceA;
    public System.Action<GameManager> applyChoiceB;
...
public static class HistoricalEventManager
{
    ...
    static void EnsurePool()
    {
        ...
        eventPool.Add(new HistoricalEvent("Congress of Vienna",
            ...
            gm => { gm.LogMessage("Congress of Vienna concludes."); })
        {
            earliestTurn = 20,
            latestTurn = 30,
            requiresPlayerChoice = true,
            choiceALabel = "Endorse the Settlement",
            choiceBLabel = "Subvert the Delegates",
            applyChoiceA = gm => {
                FactionData p = gm.GetPlayerFaction();
                if (p == null) return;
                p.legitimacy = Mathf.Min(100f, p.legitimacy + 8f);
                p.influence = Mathf.Min(100, p.influence + 5);
                p.secrets = Mathf.Max(0, p.secrets - 3);
                gm.LogMessage("Congress of Vienna: endorsed the settlement.");
            },
            applyChoiceB = gm => {
                FactionData p = gm.GetPlayerFaction();
                if (p == null) return;
                p.legitimacy = Mathf.Max(0f, p.legitimacy - 5f);
                p.secrets = Mathf.Min(50, p.secrets + 10);
                List<FactionData> enemies = gm.factions.FindAll(f => !f.isEliminated && f != p);
                if (enemies.Count > 0)
                {
                    FactionData enemy = enemies[Random.Range(0, enemies.Count)];
                    enemy.influence = Mathf.Max(0, enemy.influence - 8);
                    gm.LogMessage($"{enemy.factionName} loses 8 influence.");
                }
                gm.LogMessage("Congress of Vienna: delegates subverted.");
            }
        });
        ...
        eventPool.Add(new HistoricalEvent("Industrial Revolution",
            ...
            gm => { gm.LogMessage("Industrial Revolution settles."); })
        {
            earliestTurn = 30,
            latestTurn = 70,
            requiresPlayerChoice = true,
            choiceALabel = "Fund the Factories",
            choiceBLabel = "Infiltrate the Unions",
            applyChoiceA = gm => {
                FactionData p = gm.GetPlayerFaction();
                if (p == null) return;
                p.gold = Mathf.Max(0, p.gold - 10);
                p.power = Mathf.Min(100, p.power + 15);
                p.goldIncomeBonus += 2;
                gm.LogMessage("Industrial Revolution: factories funded.");
            },
            applyChoiceB = gm => {
                FactionData p = gm.GetPlayerFaction();
                if (p == null) return;
                p.influence = Mathf.Min(100, p.influence + 12);
                p.secrets = Mathf.Min(50, p.secrets + 3);
                foreach (FactionData f in gm.factions)
                    if (f != p && !f.isEliminated && f.power > 60)
                        f.power = Mathf.Max(0, f.power - 5);
                gm.LogMessage("Industrial Revolution: unions infiltrated.");
            }
        });
            ...
            })
        {
            ...
            latestTurn = 65,
            requiresPlayerChoice = true,
            choiceALabel = "Back the Revolutionaries",
            choiceBLabel = "Back the Old Order",
            applyChoiceA = gm => {
                FactionData p = gm.GetPlayerFaction();
                if (p == null) return;
                p.influence = Mathf.Min(100, p.influence + 10);
                p.legitimacy = Mathf.Max(0f, p.legitimacy - 5f);
                FactionData weakest = gm.factions
                    .Where(f => f != p && !f.isEliminated && f.territories.Count > 0)
                    .OrderBy(f => f.territories.Count)
                    .FirstOrDefault();
                if (weakest != null)
                {
                    TerritoryData gained = weakest.territories.OrderBy(t => t.troops).FirstOrDefault();
                    if (gained != null)
                    {
                        weakest.territories.Remove(gained);
                        gained.controlledBy = p.factionId;
                        gained.troops = Mathf.Max(1, gained.troops);
                        p.territories.Add(gained);
                        MapSystemController.Instance?.SyncProvinceFromTerritory(gained.name);
                        gm.LogMessage($"Revolutionaries hand {gained.name} to {p.factionName}.");
                    }
                }
            },
            applyChoiceB = gm => {
                ...
                if (p == null) return;
                p.legitimacy = Mathf.Min(100f, p.legitimacy + 8f);
                p.power = Mathf.Min(100, p.power + 5);
                foreach (FactionData f in gm.factions)
                    if (f != p && !f.isEliminated)
                        f.influence = Mathf.Min(100, f.influence + 3);
                gm.LogMessage("The old order is backed; rival factions feed on the chaos.");
            }
        });
    ...
    public static bool TryTriggerEvent(GameManager gm)
    {
        ...
        if (!activeEvent.requiresPlayerChoice)
            activeEvent.applyEffect?.Invoke(gm);
        FactionData carbonari = gm.factions.Find(f => f.factionId == 3);
        if (carbonari != null && !carbonari.isEliminated)
        {
            carbonari.influence = Mathf.Min(100, carbonari.influence + 5);
            gm.LogMessage("Carbonari Revolutionary Cell: +5 influence from historical chaos.");
        }
        gm.LogMessage($"\u26A1 HISTORICAL EVENT [{gm.CurrentYear} AD]: {activeEvent.eventName} \u2014 {activeEvent.description}");
        return true;
    }

    public static void ResolveChoice(GameManager gm, bool isChoiceA)
    {
        if (gm == null || activeEvent == null || !activeEvent.requiresPlayerChoice) return;
        if (isChoiceA)
            activeEvent.applyChoiceA?.Invoke(gm);
        else
            activeEvent.applyChoiceB?.Invoke(gm);
    }

    public static bool TryTriggerCrisisEvent(GameManager gm)
    {
        if (gm == null || gm.factions == null || gm.territories == null) return false;

        FactionData exposed = gm.factions.Find(f => !f.isEliminated && f.publicExposure > 70f);
        if (exposed != null)
        {
            exposed.secrets = Mathf.Max(0, exposed.secrets - 10);
            List<TerritoryData> owned = gm.territories.FindAll(t => t.controlledBy == exposed.factionId);
            if (owned.Count > 0)
                owned[Random.Range(0, owned.Count)].RemoveTroops(2);
            gm.LogMessage($"CRISIS: Informant Exposed. {exposed.factionName} loses 10 secrets and agents.");
            return true;
        }

        FactionData richest = gm.factions
            .Where(f => !f.isEliminated && f.gold > 25)
            .OrderByDescending(f => f.gold)
            .FirstOrDefault();
        if (richest != null)
        {
            int lost = richest.gold / 2;
            richest.gold -= lost;
            gm.LogMessage($"CRISIS: Treasury Raid. {richest.factionName} loses {lost} gold.");
            return true;
        }

        List<FactionData> secretive = gm.factions
            .Where(f => !f.isEliminated && f.secrets > 30)
            .OrderByDescending(f => f.secrets)
            .Take(2)
            .ToList();
        if (secretive.Count == 2)
        {
            secretive[0].secrets = Mathf.Max(0, secretive[0].secrets - 2);
            secretive[1].secrets = Mathf.Max(0, secretive[1].secrets - 2);
            gm.LogMessage($"CRISIS: Double Agent. {secretive[0].factionName} and {secretive[1].factionName} trade compromised secrets.");
            return true;
        }

        if (gm.currentTurn > 20 && Random.value < 0.05f && gm.territories.Count > 0)
        {
            TerritoryData target = gm.territories[Random.Range(0, gm.territories.Count)];
            target.RemoveTroops(2);
            target.skipReinforceNextTurn = true;
            gm.LogMessage($"CRISIS: Plague Outbreak. {target.name} loses 2 troops and cannot be reinforced next turn.");
            return true;
        }

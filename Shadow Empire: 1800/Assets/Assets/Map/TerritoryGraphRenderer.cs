...
public class TerritoryGraphRenderer : MonoBehaviour
{
    ...
    public static void ClearPositions() { positions = null; }
    ...
    static void EnsurePositions()
    {
        ...
        foreach (TerritoryData t in GameManager.Instance.territories)
            positions[t.name] = t.mapPosition;
    }
    ...
    public static Vector2 GetCanvasAnchor(string name)
    {
        ...
        // NodeLayer and arrowContainer both fill MapImage, so use the same
        // normalized map-space anchors as MapLayerController.BuildNodeRoots().
        float nx = territory.mapPosition.x;
        float ny = 1f - territory.mapPosition.y;

        // The container is full-stretch over the map image. We need the
        ...
        if (Instance != null && Instance.arrowContainer != null)
        {
            ...
            // nx, ny are fractions of the map image; the container maps [0,1]
            // to the full map width/height. So pixel offset from lower-left:
            float px = nx * r.width;

...
public class GameUIManager : MonoBehaviour
{
    ...
    static readonly Color Ink       = new Color(0.05f, 0.03f, 0.02f);
    static readonly Color Darkest   = new Color(0.10f, 0.05f, 0.02f);
    static readonly Color Dark      = new Color(0.16f, 0.08f, 0.02f);
    static readonly Color Med       = new Color(0.24f, 0.11f, 0.03f);
    static readonly Color Gold      = new Color(0.83f, 0.65f, 0.22f);
    static readonly Color Parchment = new Color(0.91f, 0.86f, 0.78f);
    static readonly Color Rust      = new Color(0.45f, 0.25f, 0.10f);
    static readonly Color DimGold   = new Color(0.55f, 0.40f, 0.08f);
    static readonly Vector2 MapAnchorMin = new Vector2(0f, 0.04f);
    static readonly Vector2 MapAnchorMax = new Vector2(0.74f, 0.97f);
    ...
    void Start()
    {
        // Clean up stale canvases from previous sessions.
        // MenuCanvas is NOT touched — it belongs to MainMenuManager.
    ...
    void CreateCanvas()
    {
        ...
        if (Camera.main != null)
        {
            Camera.main.backgroundColor = new Color(0.08f, 0.10f, 0.14f);
    ...
    void BuildUI()
    {
        ...
        topBar.AddComponent<Image>().color = Ink;
        topBar.GetComponent<Image>().raycastTarget = false;
        Anchor(topBar.GetComponent<RectTransform>(), 0, 0, 1, 0.05f);

        MakeHRule(topBar.transform, new Vector2(0, 1), new Vector2(1, 0.96f), Gold.WithA(0.35f));
        ...
        phaseTimerBar.color = new Color(0.3f, 0.7f, 0.3f, 0.6f);
        ...
        mtR.anchorMin = new Vector2(0.88f, 0.02f);
        mtR.anchorMax = new Vector2(0.97f, 0.95f);
        ...
        ecR.anchorMin = new Vector2(0.79f, 0.02f);
        ecR.anchorMax = new Vector2(0.87f, 0.95f);
        ...
        mapBgImg.color = new Color(0.05f, 0.03f, 0.02f, 0.85f);
        mapBgImg.raycastTarget = false;
        Texture2D europeMap = Resources.Load<Texture2D>("EuropeMap");
        if (europeMap != null)
        {
            mapBgImg.sprite = Sprite.Create(europeMap, new Rect(0, 0, europeMap.width, europeMap.height), new Vector2(0.5f, 0.5f));
            mapBgImg.preserveAspect = false;
            mapBgImg.color = Color.white;
        }
        else
        {
            Debug.LogWarning("[GameUIManager] EuropeMap texture not found in Resources.");
        }
        ...
        Outline frameOutline = mapFrame.AddComponent<Outline>();
        frameOutline.effectColor = new Color(0.78f, 0.58f, 0.16f, 0.6f);
        ...
        Anchor(factionArea.GetComponent<RectTransform>(), 0.735f, 0.30f, 0.998f, 0.98f);
        ...
        logBg.color = new Color(0.06f, 0.03f, 0.015f, 0.90f);
        logBg.raycastTarget = false;
        Anchor(logArea.GetComponent<RectTransform>(), 0.735f, 0.06f, 0.998f, 0.28f);
        ...
        ctText.font = GetFont();
        ctText.fontSize = 9;
        ctText.color = new Color(0.80f, 0.75f, 0.65f);
        ...
        endTurnButtonImage.color = new Color(0.55f, 0.18f, 0.08f);
        ...
        Anchor(endTurnRoot.GetComponent<RectTransform>(), 0.73f, 0.06f, 1.0f, 0.14f);
        endTurnLabel = MakeUIText(endTurnRoot.transform, "Label", 14, FontStyle.Bold,
        ...
        Anchor(cardsRemainingRoot.GetComponent<RectTransform>(), 0.73f, 0.12f, 0.99f, 0.17f);
        cardsRemainingText = cardsRemainingRoot.AddComponent<Text>();
        cardsRemainingText.font = GetFont();
        cardsRemainingText.fontSize = 10;
        cardsRemainingText.fontStyle = FontStyle.Bold;
        cardsRemainingText.alignment = TextAnchor.MiddleLeft;
    ...
    void BuildTerritoryMapPanel(Transform ct)
    {
        ...
        zoomCtrl.Setup(canvas, territoryMapPanel.GetComponent<RectTransform>());
    }
    ...
    void OnTerritoryClicked(TerritoryData territory)
    {
        ...
        if (GameManager.Instance.IsWaitingForReinforcements)
        {
            ...
            if (player != null && (territory.controlledBy == player.factionId || territory.controlledBy == -1))
            {
                if (territory.controlledBy == -1)
                {
                    territory.controlledBy = player.factionId;
                    if (!player.territories.Contains(territory)) player.territories.Add(territory);
                    MapSystemController.Instance?.SyncProvinceFromTerritory(territory.name);
                }
                GameManager.Instance.AssignReinforcementToTerritory(territory, 1);
            }
            else
            {
                AddLogMessage("You can only reinforce your own or neutral territories.");
            }
            return;
        }
        ...
        // ADDED: UPGRADE 2 - show dice choice before target selection
    ...
    {
        return new Vector2(
            Mathf.Lerp(MapAnchorMin.x, MapAnchorMax.x, Mathf.Clamp01(rawUV.x)),
            Mathf.Lerp(MapAnchorMin.y, MapAnchorMax.y, 1f - Mathf.Clamp01(rawUV.y)));
    }
    ...
    void BuildMapClickZones(Transform ct)
    {
        ...
        FullStretch(mapClickZonesRoot.GetComponent<RectTransform>());
    ...
    void BuildReinforcementPanel(Transform ct)
    {
        ...
        Anchor(reinforcementPanel.GetComponent<RectTransform>(), 0.735f, 0.14f, 0.998f, 0.30f);
        ...
        MakeUIText(reinforcementPanel.transform, "Title", 14, FontStyle.Bold,
                   Gold, new Vector2(0.05f, 0.65f), new Vector2(0.95f, 0.90f))
            .GetComponent<Text>().text = "REINFORCEMENT PHASE";

        reinforcementRemainingText = MakeUIText(reinforcementPanel.transform, "Remaining", 11, FontStyle.Normal,
                                                 Parchment, new Vector2(0.05f, 0.45f), new Vector2(0.95f, 0.60f)).GetComponent<Text>();

        MakeUIText(reinforcementPanel.transform, "Instruction", 9, FontStyle.Normal,
                   Parchment.WithA(0.65f), new Vector2(0.05f, 0.25f), new Vector2(0.95f, 0.40f))
            .GetComponent<Text>().text = "Click owned territories to place";
    ...
    IEnumerator ShowPhaseBanner(string label, Color color)
    {
        ...
        yield return StartCoroutine(AutoHideBanner());
        phaseBannerCoroutine = null;
    }

    IEnumerator AutoHideBanner()
    {
    ...
    void OnHandCardClicked(CardData card)
    {
        ...
        if (!GameManager.Instance.CanPlayMoreCards()) { AddLogMessage("Maximum 3 cards per turn."); StartCoroutine(ShakeCard(FindCardGO(card))); return; }

        switch (card.cardType)
        {
            case CardType.ATTACK:
                EnterTerritoryAttackSourceMode(card);
                break;
    ...
    bool IsValidFortifyDestination(TerritoryData source, TerritoryData destination, int factionId)
    {
        ...
        while (open.Count > 0)
        {
            ...
            foreach (string adjName in current.adjacentTerritories)
            {
    ...
    {
        ShowPhase("\u2694  REINFORCEMENT PHASE", new Color(0.3f, 0.5f, 0.8f));
        UpdateTerritoryMap();
    }
    ...
    void ShowPhase(string label, Color color)
    {
        ...
        phaseBannerCoroutine = StartCoroutine(ShowPhaseBanner(label, color));
    }
    ...
    void ApplyPhaseVisibility(GameState state)
    {
        ...
        SetActiveIfExists(mapClickZonesRoot, tacticalMapPhase);
        SetActiveIfExists(factionArea, shellVisible);
        SetActiveIfExists(logArea, shellVisible);
        SetActiveIfExists(handArea, drawPhase || playPhase);
    ...
    public void OnPhaseTimerUpdate(float timeLeft)
    {
        ...
        if (timeLeft > 20f)
            phaseTimerBar.color = new Color(0.25f, 0.65f, 0.25f, 0.75f);
        else if (timeLeft > 10f)
            phaseTimerBar.color = new Color(0.75f, 0.65f, 0.15f, 0.75f);
        else
        {
            float pulse = Mathf.PingPong(Time.unscaledTime * 4f, 0.3f) + 0.7f;
            phaseTimerBar.color = new Color(0.80f, 0.15f, 0.08f, pulse);
        }
    }
    ...
    void BuildTerritoryTooltip(Transform ct)
    {
        ...
        tr.sizeDelta = new Vector2(180, 90);
        ...
        territoryTooltipName = MakeUIText(territoryTooltip.transform, "Name", 12, FontStyle.Bold,
            Parchment, new Vector2(0.05f, 0.74f), new Vector2(0.95f, 0.96f)).GetComponent<Text>();

        territoryTooltipOwner = MakeUIText(territoryTooltip.transform, "Owner", 9, FontStyle.Normal,
            Parchment.WithA(0.75f), new Vector2(0.05f, 0.56f), new Vector2(0.95f, 0.68f)).GetComponent<Text>();

        territoryTooltipStats = MakeUIText(territoryTooltip.transform, "Stats", 8, FontStyle.Normal,
    ...
    {
        if (territoryTooltip == null) return;
        ...
        territoryTooltipOwner.text = $"Controlled by: {ownerName}";
        territoryTooltipStats.text = $"Troops: {t.troops}  Farm: {t.farmingValue}  Strat: {t.strategicValue}";
    ...
    void BuildCardPrefab()
    {
        ...
        // FIX ERROR-1: CardUIController.Awake() already adds CanvasGroup.
        // Use GetComponent first to avoid the "already added" warning.
    ...
    void BuildFactionPanelPrefab()
    {
        ...
        Image stripeImg = MakeChild(factionPanelPrefab.transform, "AccentStripe", new Vector2(0, 0.88f), Vector2.one, Vector2.zero)
        .AddComponent<Image>();
        stripeImg.color = Color.gray;
    ...
    public void FlashTerritory(string territoryName)
    {
        ...
        Vector2 anchor = TerritoryUvToAnchor(uv);
